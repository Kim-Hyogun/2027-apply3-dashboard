...
def format_delta(val):
    ...
    if val > 0:
        return f"+{val:,}", "delta-up"
    elif val < 0:
        return f"{val:,}", "delta-down"
    return "0", "delta-neutral"

in_delta_val, in_delta_class = format_delta(totals["정원내_전일대비"])
out_delta_val, out_delta_class = format_delta(totals["정원외_전일대비"])
total_delta_val, total_delta_class = format_delta(totals["계_전일대비"])
...
with col1:
    ...
    <div class="metric-card">
        ...
        <div class="metric-delta {in_delta_class}">전일 대비: {in_delta_val}</div>
    </div>
    """, unsafe_allow_html=True)
...
with col2:
    ...
    <div class="metric-card">
        ...
        <div class="metric-delta {out_delta_class}">전일 대비: {out_delta_val}</div>
    </div>
    """, unsafe_allow_html=True)
...
with col3:
    ...
    <div class="metric-card">
        ...
        <div class="metric-delta {total_delta_class}">전일 대비: {total_delta_val}</div>
    </div>
    """, unsafe_allow_html=True)
...
with chart_col1:
    st.markdown("### 📈 일차별 누적 지원자 추이 (올해 vs 작년)")
    fig_trend = go.Figure()
    # Current year line
...
with chart_col2:
    st.markdown("### 🏆 정원내 경쟁률 상위 TOP 10 학과")
    # Calculate competition rate for each department
    df_comp = df.copy()
    df_comp["경쟁률"] = df_comp.apply(
        lambda row: (row["정원내_지원"] / row["정원내_모집"]) if row["정원내_모집"] > 0 else 0.0,
        axis=1
    )
    df_top10 = df_comp.sort_values(by="경쟁률", ascending=True).tail(10)  # Ascending for horizontal bar
